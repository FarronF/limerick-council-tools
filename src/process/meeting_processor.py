...
def process_meeting(meeting_folder: str, output_meeting_folder: str, file_filter: List[str] = None):
        ...
        for file_info in files_info:
            ...
            if downloaded & (file_name.endswith(".pdf")) & (file_filter is None or any(filter_word.lower() in file_name.lower() for filter_word in file_filter)):
                ...
                if os.path.exists(pdf_file_path):
                    processed = _process_pdf(pdf_file_path, output_meeting_folder, file_url)
...
def _process_pdf(pdf_path, output_folder, original_url):
    print(f"📄 Processing PDF: {os.path.basename(pdf_path)}")
    try:
        fitz_processor = FitzProcessor(ocr_log_file_path = os.path.join(".logs", "ocr_log.txt"))
        markdown_output = fitz_processor.process(pdf_path)
        if markdown_output:
            markdown_output = f"[Original file]({original_url})\n\n---\n" + markdown_output
            print(f"OUTPUT FOLDER: {output_folder}")
            _save_markdown(markdown_output, pdf_path, output_folder)
        else:
...
def _save_markdown(markdown_content, pdf_path, output_folder):
